Check_ForUpdate(url = "", _SuppressMsgBox = 3, _ReplaceCurrentScript = 1, _CallbackFunction = "", ByRef _Information = "")
{
	
	
	;gemod door Noeël Moeskops v1 orgineel door Rseding91 v1.7
	;deze versie is zo aangepast dat hij kan worden gebruikt als libary
	;ook zijn er meer error berichten in gedaan
	...
	;parameter 1:URL 
	;de update url naar de version.ini bestand

	;parameter 2: _SuppressMsgBox = 0
	;About: 
	; als hij op 0 staat vraagt hij om alles en de extra error berichten die ik heb toegevoegt
	;If set to 1, the function will skip asking if the user wants to update and simply attempt to update (if there is a newer version available).
	...
	;parameter 3: _ReplaceCurrentScript = 1
	...
	;parameter 5: ByRef _Information = ""
	...
	;je moet ook een proNaam en versie variable hebben in het hoofd script om de naam en versie aan te geven
	global proNaam
	global versie
	 Script_Name := proNaam ;Your script name
	, Version_Number := versie ;The script's version number
	, Update_URL := url ;The URL of the version.ini file for your script
	...
	Loop,% Retry_Count
	{
		...
		if (Version == Version_Number && _SuppressMsgBox == 0){
			MsgBox,, %proNaam%, je hebt al de hoogste versie: %Version%
		}
		If (Version = "N/A"){
			...
			If (A_Index = Retry_Count)
			{
				...
				if (_SuppressMsgBox == 0)
				{
					MsgBox,,%proNaam%,  kan de update server niet berijken
				}
			}
			Else
				Sleep,500
		...
		If (Version > Version_Number){
			If (_SuppressMsgBox != 1 and _SuppressMsgBox != 3){
				MsgBox,0x4,%proNaam%,Er is een nieuwe versie van %Script_Name% beschrikbaar.`nHuidige versie: %Version_Number%`nNieuwe versie: %Version%`n`nWil je nu updaten?
	...
	If (_ReplaceCurrentScript And Success){
		...
		If (_SuppressMsgBox < 2)
		{
			If (InStr(VBS_P2, "Do Until (Finished = True Or Count = 5)"))
			{
				...
				VBS_P3 .= "`r`nWScript.Echo ""Update mislukt."""
				VBS_P3 .= "`r`nElse"
				If (Extension != "exe")
					VBS_P3 .= "`r`nobjFSO.DeleteFile """ A_ScriptFullPath """"
				VBS_P3 .= "`r`nWScript.Echo ""Update succesvol."""
				VBS_P3 .= "`r`nEnd If"
			} Else
				VBS_P3 .= "`r`nWScript.Echo ""Update succesvol."""
		}
